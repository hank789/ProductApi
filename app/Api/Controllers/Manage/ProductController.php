...
class ProductController extends Controller {
    ...
    public function updateCase(Request $request) {
        ...
        if ($data['type'] == 'link' && $data['link_url'] != $case->content['link_url']) {
            ...
            $article_uuid = base64_encode($linkInfo['wxHao'].$linkInfo['title'].date('Y-m-d',strtotime($linkInfo['date'])));
            ...
            if ($aid) {
                ...
                $article = WechatWenzhangInfo::create([
                    ...
                    'description' => '',
                    'date_time'   => $linkInfo['date'],
                    'mp_id' => $linkInfo['mp_id']??0,
    ...
    public function delSource(Request $request) {
        ...
        Client::instance()->request(config('app.partner_service_url').'/api/partner/service/removeSource',['source_id'=>$model->sort]);
        return self::createJsonData(true);
    }
    ...
    public function storeSource(Request $request) {
        ...
        $res = Client::instance()->request(config('app.partner_service_url').'/api/partner/service/addSource',['source'=>$source,'source_id'=>$source_id]);
        if ($res && $res['code'] == 1000) {
            $mpInfo = WechatMpInfo::find($res['data']['_id']);
            if (!$mpInfo) {
                $mpInfo = WechatMpInfo::create($res['data']);
            }
        } else {
    ...
    public function fetchSourceInfo(Request $request) {
        ...
        $res = Client::instance()->request(config('app.partner_service_url').'/api/partner/service/fetchSourceInfo',['source'=>$wx_hao]);
        if ($res && $res['code'] == 1000) {
            $mpInfo = WechatMpInfo::create($res['data']);
        }
